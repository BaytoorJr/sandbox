...
-- Need to drop unique_company_uin_name
DO $$
    BEGIN
        IF NOT EXISTS (
            SELECT 1
...
CREATE OR REPLACE FUNCTION check_unique_account_globally()
    RETURNS TRIGGER AS $$
BEGIN
    -- Проверка уникальности счета в рамках company_id
    IF EXISTS (
        SELECT 1
        FROM counterparty_accounts ca
                 JOIN counterparty c ON ca.counterparty_id = c.id
        WHERE ca.number = NEW.number
          AND c.company_id = (SELECT company_id FROM counterparty WHERE id = NEW.counterparty_id)
          AND ca.id <> NEW.id
    ) THEN
        RAISE EXCEPTION USING ERRCODE = '23505',
            MESSAGE = 'Account number ' || NEW.number || ' already exists for the company';
    END IF;
...
DO $$
    BEGIN
        IF EXISTS (
            ...
            ALTER TABLE counterparty
                DROP CONSTRAINT unique_company_uin_name;
            ...
            IF NOT EXISTS (
                ...
                FROM pg_constraint cons
                         JOIN pg_class tbl ON cons.conrelid = tbl.oid
                         JOIN pg_namespace ns ON tbl.relnamespace = ns.oid
                WHERE cons.conname = 'unique_company_uin'
                  AND ns.nspname = 'public'  -- Replace with your schema if not 'public'
            ) THEN
                -- Add the new constraint
                ALTER TABLE counterparty
                    ADD CONSTRAINT unique_company_uin UNIQUE (company_id, uin);
            END IF;
